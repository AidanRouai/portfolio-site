...
export default function CardsRendering() {
  ...
  useEffect(() => {
    cards.forEach((_, i) => 
        {setTimeout(() => {setVisibleCards(prev => [...prev, i])}, i * 100) // 300ms delay between each card
})
  }, [])
  ...
  return (
    <div className="flex flex-row space-x-8 justify-center">
      {cards.map((card, i) => (
        <div key={i} className={`transition-all duration-500 ${ visibleCards.includes(i)? 'opacity-100 rotate-y-0': 'opacity-0 rotate-y-90'}`}
          style={{
            transform: visibleCards.includes(i) ? 'rotateY(0deg)' : 'rotateY(90deg)',
